...
export function createAgentTool(ctl: ToolsProviderController) {
  const config = ctl.getPluginConfig(configSchematics);
  const agentMode = config.get("operationMode") as boolean;
  ...
  return tool({
    ...
    `,
    parameters: {
      task_description: z.string().describe("Detailed description of the development task to complete autonomously"),
      scope: z.string().optional().describe("Optional scope or constraints for the task")
    },
